...
mod lumen {
    use super::*;
    use crate::{Lumen, SimpleHashFiatShamirRng};

    use ark_bls12_381::{Bls12_381, Fr};
    use ark_ff::UniformRand;
    use ark_poly::univariate::DensePolynomial;
    use ark_poly_commit::lumen_pc::LumenKZG10;
    use ark_std::ops::MulAssign;
    use blake2::Blake2s;
    use rand_chacha::ChaChaRng;

    type MultiPC = LumenKZG10<Bls12_381, DensePolynomial<Fr>>;
    type FS = SimpleHashFiatShamirRng<Blake2s, ChaChaRng>;
    type LumenInst = Lumen<Fr, MultiPC, FS>;

    fn test_circuit(num_constraints: usize, num_variables: usize) {
        let rng = &mut ark_std::test_rng();

        let universal_srs = LumenInst::universal_setup(100, 25, 300, rng).unwrap();
        ...
        for _ in 0..100 {
            ...
            let (index_pk, index_vk) = LumenInst::index(&universal_srs, circ.clone()).unwrap();
            println!("Called index");

            let proof = LumenInst::prove(&index_pk, circ, rng).unwrap();
            println!("Called prover");

            assert!(LumenInst::verify(&index_vk, &[c, d], &proof, rng).unwrap());
            println!("Called verifier");
            println!("\nShould not verify (i.e. verifier messages should print below):");
            assert!(!LumenInst::verify(&index_vk, &[a, a], &proof, rng).unwrap());
        }
    }
    ...
    fn prove_and_test_outlining() {
        ...
        let universal_srs = LumenInst::universal_setup(150, 150, 150, rng).unwrap();
        ...
        assert!(LumenInst::verify(&index_vk, &inputs, &proof, rng).unwrap());
        println!("Called verifier");
    }
}
