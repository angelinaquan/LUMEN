...
/// Implements an Polynomial Interactive Oracle Proof (IOP).
pub mod iop;
pub use iop::iopForR1CS;
use iop::EvaluationsProvider;

#[cfg(test)]
mod test;

/// The compiled argument system.
pub struct Lumen<F: PrimeField, PC: PolynomialCommitment<F, DensePolynomial<F>>, FS: FiatShamirRng>(
    ...
    Lumen<F, PC, FS>
{
    ...
    /// Fiat-Shamir rng.
    pub const PROTOCOL_NAME: &'static [u8] = b"Lumen-2023";
    ...
    pub fn universal_setup<R: RngCore>(
        ...
        let max_degree = iopForR1CS::<F>::max_degree(num_constraints, num_variables, num_non_zero)?;
        let setup_time = start_timer!(|| {
            format!(
            "Lumen::UniversalSetup with max_degree {}, computed for a maximum of {} constraints, {} vars, {} non_zero",
    ...
    pub fn index<C: ConstraintSynthesizer<F>>(
        ...
        let index_time = start_timer!(|| "Lumen::Index");

        // TODO: Add check that c is in the correct mode.
        let index = iopForR1CS::index(c)?;
        if srs.max_degree() < index.max_degree() {
            Err(Error::IndexTooLarge)?;
        }

        let coeff_support = iopForR1CS::get_degree_bounds(&index.index_info);
        // Lumen only needs degree 2 random polynomials
    ...
    pub fn prove<C: ConstraintSynthesizer<F>, R: RngCore>(
        ...
        let prover_time = start_timer!(|| "Lumen::Prover");
        // Add check that c is in the correct mode.

        let prover_init_state = iopForR1CS::prover_init(&index_pk.index, c)?;
        ...
        let (prover_first_msg, prover_first_oracles, prover_state) =
            iopForR1CS::prover_first_round(prover_init_state, zk_rng)?;
        ...
        let (verifier_first_msg, verifier_state) =
            iopForR1CS::verifier_first_round(index_pk.index_vk.index_info, &mut fs_rng)?;
        // --------------------------------------------------------------------
        ...
        let (prover_second_msg, prover_second_oracles, prover_state) =
            iopForR1CS::prover_second_round(&verifier_first_msg, prover_state, zk_rng);
        ...
        let (verifier_second_msg, verifier_state) =
            iopForR1CS::verifier_second_round(verifier_state, &mut fs_rng);
        // --------------------------------------------------------------------

        // --------------------------------------------------------------------
        // Third round
        let (prover_third_msg, prover_third_oracles) =
            iopForR1CS::prover_third_round(&verifier_second_msg, prover_state, zk_rng)?;
        ...
        let verifier_state = iopForR1CS::verifier_third_round(verifier_state, &mut fs_rng);
        // --------------------------------------------------------------------
        ...
        let labeled_comms: Vec<_> = index_pk
            ...
            .iter()
            .cloned()
            .zip(&iopForR1CS::<F>::INDEXER_POLYNOMIALS)
        ...
        // Compute the IOP verifier's query set.
        let (query_set, verifier_state) =
            iopForR1CS::verifier_query_set(verifier_state, &mut fs_rng);
        let lc_s = iopForR1CS::construct_linear_combinations(
            &public_input,
            &polynomials,
            &verifier_state,
        )?;
        ...
        for (label, (_, point)) in &query_set {
            let lc = lc_s
                ...
                .ok_or(iop::Error::MissingEval(label.to_string()))?;
            let eval = polynomials.get_lc_eval(&lc, *point)?;
            if !iopForR1CS::<F>::LC_WITH_ZERO_EVAL.contains(&lc.label.as_ref()) {
    ...
    pub fn verify<R: RngCore>(
        ...
        let verifier_time = start_timer!(|| "Lumen::Verify");
        ...
        let (_, verifier_state) =
              iopForR1CS::verifier_first_round(index_vk.index_info, &mut fs_rng)?;
        // --------------------------------------------------------------------
        ...
        let (_, verifier_state) = iopForR1CS::verifier_second_round(verifier_state, &mut fs_rng);
        // --------------------------------------------------------------------
        ...
        let degree_bounds = vec![None; index_vk.index_comms.len()]
            .into_iter()
            .chain(iopForR1CS::prover_first_round_degree_bounds(&index_info))
            .chain(iopForR1CS::prover_second_round_degree_bounds(&index_info))
            .chain(iopForR1CS::prover_third_round_degree_bounds(&index_info))
            .collect::<Vec<_>>();
        ...
        let commitments: Vec<_> = index_vk
            ...
            .chain(third_comms)
            .cloned()
            .zip(iopForR1CS::<F>::polynomial_labels())
            .zip(degree_bounds)
        ...
        for (poly_label, (_, point)) in query_set.iter().cloned() {
            if iopForR1CS::<F>::LC_WITH_ZERO_EVAL.contains(&poly_label.as_ref()) {
        ...
        end_timer!(verifier_time, || format!(
            " PC::Check for IOP  Verifier linear equations: {}",
