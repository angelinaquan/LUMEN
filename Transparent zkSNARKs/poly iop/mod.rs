...
/// Describes data structures and the algorithms used by the IOP indexer.
pub mod indexer;
/// Describes data structures and the algorithms used by the IOP prover.
pub mod prover;
/// Describes data structures and the algorithms used by the IOP verifier.
pub mod verifier;
...
/// Currently, this IOP only supports inputs of size one
/// less than a power of 2 (i.e., of the form 2^n - 1).
pub struct iopForR1CS<F: Field> {
    field: PhantomData<F>,
}

impl<F: PrimeField> iopForR1CS<F> {
    /// The labels for the polynomials output by the IOP indexer.
    #[rustfmt::skip]
    ...
    /// The labels for the polynomials output by the IOP prover.
    #[rustfmt::skip]
    ...
    /// Get all the strict degree bounds enforced in the IOP.
    ...
    /// Construct the linear combinations that are checked by the IOP.
...
/// when constructing linear combinations via `iopForR1CS::construct_linear_combinations`.
...
/// Describes the failure modes of the IOP protocol.
#[derive(Debug)]
pub enum Error {
